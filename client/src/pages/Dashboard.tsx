...
const LockedSection = ({
  ...
  <div
    id={id}
    className="flex flex-col items-center justify-center p-12 md:p-32 rounded-[40px] bg-[#1a1a1a] text-center mb-8"
  >
    <div className="p-6 bg-[#262626] rounded-full mb-6">
      <Lock size={24} className="text-white-500" strokeWidth={1.5} />
    </div>
    <h3 className="text-xl font-bold text-white mb-2">{title}</h3>
    <p className="text-gray-300 font-regular text-sm leading-relaxed max-w-md">
      {message}
    </p>
  </div>
);
...
export default function Dashboard() {
  const { transactions, budgets, currentMonth, isLoading, setTransactions } =
    useFinance();
  ...
  const hasBudgetsSet =
    budgets && Object.values(budgets).some((val) => Number(val) > 0);
  ...
  useEffect(() => {
    ...
    const fromMonzo =
      document.referrer.includes("monzo.com") ||
    ...
    if (fromMonzo || pendingAuth === "true") {
      ...
      setAlert({
        ...
        message:
          "Authenticating with Monzo... Please check your mobile app to approve.",
        isAuthError: true,
      });
      syncMonzoData();
    }
  ...
  const syncMonzoData = async () => {
    ...
    if (isAuthorized) {
      setAlert({
        ...
        title: "Up to Date",
        message:
          "Monzo Synced successfully. You have no new transactions to review.",
      });
      return;
    }
  ...
  const onConfirmSuccess = async (confirmedData: Transaction[]) => {
    ...
    if (import.meta.env.VITE_DEMO_MODE === "true") {
      setAlert({
        type: "success",
        title: "Demo Mode",
        message: "Transactions processed locally.",
      });
      return;
    }
    ...
    try {
      ...
      setAlert({
        ...
        message: "Reviewed and saved successfully.",
      });
    } catch (err) {
      console.error(err);
    }
  ...
  if (isLoading)
    return (
      <div className="fixed inset-0 flex items-center justify-center bg-app-bg">
        <svg
          className="w-10 h-10 animate-spin fill-emerald-500"
          viewBox="0 0 100 101"
          fill="none"
        >
          <path
  ...
  return (
    <>
          ...
          <div className="relative w-full max-w-xl animate-in zoom-in-95 duration-200">
            <div
              className={`rounded-xl border p-5 ${
                alert.type === "error"
                  ? "bg-[#1a0a0a] border-red-500/50"
                  : "bg-[#0a1a11] border-emerald-500/50"
              }`}
            >
                  ...
                  {alert.isAuthError && (
                    <div className="mt-4 flex gap-3">
                      <button
                        ...
                        className="text-xs font-regular px-3 py-1.5 bg-red-500/10 hover:bg-red-500/20 text-red-500 rounded-lg"
                      >
                        Try Again
                      </button>
                      <button
                        onClick={restartAuth}
                        className="text-xs font-regular px-3 py-1.5 bg-white/5 hover:bg-white/10 text-gray-300 rounded-lg border border-white/10"
                      >
                        Restart Auth
                      </button>
                    </div>
      ...
      <nav
        className={`sticky top-0 z-50 w-full bg-app-bg/80 backdrop-blur-md border-b border-[#222] transition-all duration-300 ${
          isAnyModalOpen ? "blur-sm brightness-50" : ""
        }`}
      >
            ...
            {isFullyUnlocked && (
              <div className="hidden md:flex items-center gap-6 animate-in fade-in slide-in-from-left-4 duration-500">
                <button
                  ...
                  className="text-xs font-regular text-gray-300 hover:text-white transition-colors"
                >
                  Overview
                </button>
                <button
      ...
      >
        <header className="mb-12 pb-8">
          <div className="flex flex-row items-stretch gap-[20px]">
            <div className="w-full max-w-2xl flex flex-col pt-[10px] md:pt-0">
              ...
              <p className="whitespace-normal break-words text-gray-300 font-regular text-base md:text-md mb-4 pt-6 leading-relaxed">
                Simply click{" "}
                <span className="text-white font-bold">'Sync Monzo'</span> and
                authenticate with Monzo via{" "}
                <span className="text-white font-bold">e-mail</span>, then your{" "}
                <span className="text-white font-bold">mobile device</span>. In
                the background, your most recent transactions will be synced to
                your dashboard. Next, hit{" "}
                <span className="text-white font-bold">'Configure Budget'</span>{" "}
                to set your budget for this month. Complete the aforementioned
                steps to unlock your dashboard metrics, easily monitor your
                spending habits with the resulting inights, and filter your data
                however you'd like.
              </p>
              <div className="flex flex-row items-center gap-3 w-full md:w-auto">
                <button
                  onClick={syncMonzoData}
                  disabled={isSyncing}
                  className="flex-1 md:flex-none flex items-center justify-center gap-2 bg-[#1a1a1a] hover:bg-[#222] text-white px-5 py-3 rounded-2xl text-sm font-medium transition-all border border-[#262626] active:scale-95"
                >
                  <RefreshCcw
                    size={18}
                  ...
                  <span className="whitespace-nowrap">
                    {isSyncing ? "Syncing..." : "Sync Monzo"}
                  </span>
                </button>

                <button
                  onClick={() => setIsBudgetModalOpen(true)}
                  className="flex-1 md:flex-none flex items-center justify-center gap-2 bg-emerald-500 hover:bg-emerald-400 text-white px-5 py-3 rounded-2xl text-sm font-medium transition-all shadow-lg shadow-emerald-500/20 active:scale-95"
                >
                  <PieChart size={18} />
                  <span className="whitespace-nowrap">Configure Budget</span>
                </button>
              </div>
            </div>
          </div>
                    ...
                    <table className="w-full text-left table-fixed min-w-[500px]">
                      <thead>
                        <tr className="bg-[#161616] text-white">
                          <th
                            scope="col"
                            className="px-5 py-5 font-regular text-sm w-1/4 text-left"
                          >
                            Date
                          </th>
                          <th
                          ...
                          <th
                            scope="col"
                            className="px-5 py-5 font-regular text-sm w-1/4 text-right"
                          >
                            Amount
                          </th>
                        </tr>
                      ...
                      <tbody>
                        {currentTransactions.map(
                          (t: Transaction, index: number) => (
                            <tr
                              key={t._id}
                              className="odd:bg-black even:bg-[#1A1A1A] border-b border-[#222] hover:odd:bg-[#161616] hover:even:bg-black h-[72px]"
                            >
                              <td className="px-5 text-sm text-white font-regular truncate">
                                {new Date(t.created).toLocaleDateString(
                                  "en-GB"
                                )}
                              </td>
                              <td className="px-5 text-sm font-regular text-white truncate">
                                {(typeof t.merchant === "object" &&
                                  t.merchant?.name) ||
                                  t.description}
                              </td>
                              <td className="px-5 text-sm">
                                <span
                                  className={`px-3.5 py-2 rounded-2xl text-sm font-medium whitespace-nowrap border border-[#262626] ${
                                    index % 2 === 0
                                      ? "bg-[#161616]"
                                      : "bg-black"
                                  }`}
                              ...
                              <td className="px-5 text-sm text-right font-regular text-white">
                        ...
                        {Array.from({
                          length: Math.max(
                            0,
                            itemsPerPage - currentTransactions.length
                          ),
                        }).map((_, i) => (
                          <tr
                            key={`empty-${i}`}
                            className="h-[72px] border-b border-[#222]/10"
                          >
                            <td colSpan={4}>&nbsp;</td>
                          </tr>
                        ))}
                      ...
                      >
                        ...
                        {getPageNumbers().map((page, index) =>
                          page === "..." ? (
                            <span
                              key={`ellipsis-${index}`}
                              className="relative inline-flex items-center px-4 py-2 text-sm font-semibold text-gray-500 border border-[#222] bg-app-bg focus:outline-offset-0"
                            >
                              ...
                            </span>
                          ) : (
                            <button
                              ...
                              className={`relative inline-flex items-center px-4 py-2 text-sm font-semibold focus:z-20 focus:outline-offset-0 ${
                                currentPage === page
                                  ? "z-10 bg-[#161616] text-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-emerald-500"
                                  : "text-gray-300 hover:bg-[#161616] border border-[#222] hidden md:inline-flex"
                              }`}
                            >
                              {page}
                            </button>
      ...
      <div
        className={isAnyModalOpen ? "blur-md brightness-50 transition-all" : ""}
      >
        <Footer />
      </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm p-6">
          <div className="w-full max-w-4xl">
            <BudgetManager
              ...
              onSaveSuccess={handleBudgetSaveSuccess}
