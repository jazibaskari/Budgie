...
const TotalExpenses: React.FC = () => {
  ...
  const { totalSpent, totalBudget, isOverBudget } = useMemo(() => {
    ...
    const budget = Object.values(budgets || {}).reduce(
      ...
      <div className="flex flex-col items-center justify-start flex-1 p-20 text-center bg-[#0a0a0a]">
        ...
        <div className="w-full mt-6">
          <p className="text-gray-400 text-sm leading-relaxed max-w-[280px] mx-auto">
            You've spent £{totalSpent.toFixed(0)} so far in{" "}
            <span className="text-white font-bold">{displayMonth}</span>. That's{" "}
            {isOverBudget ? "over" : "within"} your total budget of{" "}
