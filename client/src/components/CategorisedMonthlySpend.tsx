...
const CategorisedMonthlySpend: React.FC = () => {
    ...
    return ALL_CATEGORIES.map(({ value, label }) => {
      const spent = transactions
        .filter(
          (t) =>
            t.category === value ||
            t.category.toLowerCase() === label.toLowerCase()
        )
  ...
  return (
    <div className="bg-[#141414] border border-[#262626] rounded-xl p-6 h-[440px] shadow-xl flex flex-col">
      <h1 className="text-2xl font-medium mb-6">Categorised Spending</h1>

      <div className="h-[320px] w-full overflow-x-auto overflow-y-hidden md:overflow-hidden no-scrollbar">
      ...
      <div className="mt-auto mb-[20px] flex items-center justify-center gap-6">
        <div className="flex items-center gap-2">
          <div className="w-3 h-3 bg-[#10b981] rounded"></div>
          <span className="text-[10px] text-gray-500 font-medium er">
            Spent
          </span>
        </div>
        <div className="flex items-center gap-2">
